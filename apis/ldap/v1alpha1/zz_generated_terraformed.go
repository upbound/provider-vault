...
func (tr *AuthBackend) GetConnectionDetailsMapping() map[string]string {
	return map[string]string{"bindpass": "spec.forProvider.bindpassSecretRef", "client_tls_key": "spec.forProvider.clientTlsKeySecretRef"}
}
...
func (tr *AuthBackendGroup) LateInitialize(attrs []byte) (bool, error) {
	params := &AuthBackendGroupParameters{}
	if err := json.TFParser.Unmarshal(attrs, params); err != nil {
		return false, errors.Wrap(err, "failed to unmarshal Terraform state parameters for late-initialization")
	}
	opts := []resource.GenericLateInitializerOption{resource.WithZeroValueJSONOmitEmptyFilter(resource.CNameWildcard)}
...
func (tr *AuthBackendUser) LateInitialize(attrs []byte) (bool, error) {
	params := &AuthBackendUserParameters{}
	if err := json.TFParser.Unmarshal(attrs, params); err != nil {
		return false, errors.Wrap(err, "failed to unmarshal Terraform state parameters for late-initialization")
	}
	opts := []resource.GenericLateInitializerOption{resource.WithZeroValueJSONOmitEmptyFilter(resource.CNameWildcard)}
