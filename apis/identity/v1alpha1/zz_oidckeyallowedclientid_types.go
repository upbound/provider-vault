...
type OidcKeyAllowedClientID struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.allowedClientId) || (has(self.initProvider) && has(self.initProvider.allowedClientId))",message="spec.forProvider.allowedClientId is a required parameter"
